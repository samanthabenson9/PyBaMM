...
class TestRandauSolver(unittest.TestCase):
    ...
    def test_model_solver(self):
        ...
        solver = pybamm.ImplicitRandauSolver(rtol=1e-8, atol=1e-8)
        t_eval = np.linspace(0, 1, 100)
    ...
    def test_model_solver_failure(self):
        ...
        solver = pybamm.ImplicitRandauSolver(extra_options_call={"regularity_check": False})
        solver_old = pybamm.ImplicitRandauSolver(
    ...
    def test_model_solver_events(self):
        ...
        solver = pybamm.ImplicitRandauSolver(mode="safe", rtol=1e-8, atol=1e-8)
        ...
        solver = pybamm.ImplicitRandauSolver(mode="safe", rtol=1e-8, atol=1e-8, dt_max=1)
        t_eval = np.linspace(0, 5, 100)
        solution = solver.solve(model, t_eval)
        np.testing.assert_array_less(solution.y[0], 1.5)
        np.testing.assert_array_less(solution.y[-1], 2.5)
        ...
        solver = pybamm.ImplicitRandauSolver(mode="old safe", rtol=1e-8, atol=1e-8)
        t_eval = np.linspace(0, 5, 100)
        solution = solver.solve(model, t_eval)
        np.testing.assert_array_less(solution.y[0], 1.5)
        np.testing.assert_array_less(solution.y[-1], 2.5)
    ...
    def test_model_step_events(self):
        ...
        # Solve
        step_solver = pybamm.ImplicitRandauSolver(rtol=1e-8, atol=1e-8)
        dt = 0.05
        time = 0
        end_time = 5
        step_solution = None
