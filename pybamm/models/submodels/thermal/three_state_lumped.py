...
class ThreeStateLumped(BaseThermal):
    ...
    **Extends:** :class:`pybamm.thermal.BaseThermal`
    ...
    def get_fundamental_variables(self):
        ...
        T_outer = pybamm.Variable("Outer cell temperature")
        T_outer_dim = param.Delta_T * T_outer + param.T_ref
        T_mid = pybamm.Variable("Middle cell temperature")
        T_mid_dim = param.Delta_T * T_mid + param.T_ref
        T_core = pybamm.Variable("Core cell temperature")
    ...
    def set_rhs(self, variables):
        ...
        self.rhs = {
            ...
            T_outer: (
                self.param.B * Q_vol_av*gamma_outer + lambda_k/gamma_mid*(T_mid -T_outer) - total_cooling_coefficient* (T_outer-T_amb)
            )
            / (self.param.C_th * self.param.rho(T_vol_av)*gamma_outer),
            T_mid: (
                self.param.B * Q_vol_av*gamma_mid + lambda_k/gamma_core*(T_core -T_mid) - lambda_k/gamma_mid*(T_mid -T_outer)
            )
            / (self.param.C_th * self.param.rho(T_vol_av)*gamma_mid) ,
            T_core: (
                self.param.B * Q_vol_av *gamma_core + self.param.B * Q_isc- lambda_k/gamma_core * (T_core - T_mid)
            )
